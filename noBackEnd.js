	...
	$.ajax({
		...
		success: function( xml ){ 
			...
			for( var i = 0; i < worksheet.length; i++ ) {
				result[i] = get_worksheets( worksheet[i] );
			}
		}
	})
  	return result;
...
function get_worksheets ( url )
{
	...
	$.ajax({
		...
		cache: false,
		error: function( state, err ){ 
			alert( 'Make sure worksheet is published! ' + err );
		},
		success: function( xml ){ 
			var rowsCounter = 0;
			$( xml ).find('entry').each( function(){
				...
				$(this).children().each( function(){
					var data = $(this);
					if ( data[0].tagName.indexOf('gsx:') == 0 )
					{
						rows[colCounter++] = data.text();
					}
					else
						return;
				});
